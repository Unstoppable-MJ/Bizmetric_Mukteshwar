...
export default function StockCard({ stock, index, isExpanded, onToggle }) {
                ...
                <div className="bg-indigo-500/5 p-4 rounded-2xl border border-indigo-500/20 flex justify-between items-center">
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        if (window.confirm(`Remove ${stock.symbol.replace(".NS", "")} from this portfolio?`)) {
                          API.delete(`portfolio-stocks/${stock.id}/`)
                            .then(() => window.location.reload())
                            .catch(err => console.error(err));
                        }
                      }}
                      className="w-10 h-10 rounded-xl bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 flex items-center justify-center transition-all border border-rose-500/20"
                      title="Delete Asset"
                    >
